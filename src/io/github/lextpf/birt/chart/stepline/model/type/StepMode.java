 ...
 * the next:
 * <ul>
 * <li><b>After</b> - hold each value until the next point's position
 * (zero-order hold): the jump happens at the <em>next</em> point.</li>
 * <li><b>Before</b> - jump to the next value at the current position: the value
 * of a point is already in force when the previous point is left.</li>
 * <li><b>Center</b> - step halfway between two points; on a category axis that
 * half-way position is the category boundary.</li>
 * </ul>
 *
 * @see io.github.lextpf.birt.chart.stepline.model.type.StepLinePackage#getStepMode()
 */
public enum StepMode implements Enumerator {
	/**
	 * The '<em><b>After</b></em>' literal object: hold each value until the next
	 * point's position (zero-order hold).
	 *
	 * @see #AFTER
	 */
	...
	/**
	 * The '<em><b>Center</b></em>' literal object: step halfway between points; on
	 * a category axis this is the category boundary.
	 *
	 * @see #CENTER
	 */
